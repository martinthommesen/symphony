defmodule SymphonyElixir.WorkspaceAndConfigTest do
  ...
  test "git worktree mode creates issue branch workspaces from configured base branch" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-workspace-git-worktree-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      write_worktree_workflow!(workspace_root)

      File.cd!(source_repo, fn ->
        assert {:ok, workspace} = Workspace.create_for_issue(%{id: "77", identifier: "MT-77"})
        assert File.exists?(Path.join(workspace, ".git"))
        assert File.read!(Path.join(workspace, "README.md")) == "# Source\n"
        assert {"symphony/issue-77\n", 0} = System.cmd("git", ["-C", workspace, "branch", "--show-current"])
      end)
    after
      File.rm_rf(test_root)
    end
  ...
  test "git worktree mode applies configured dirty workspace policy" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-workspace-dirty-policy-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      workspace_root = Path.join(test_root, "workspaces")
      create_source_repo!(source_repo)
      write_worktree_workflow!(workspace_root, reset_dirty_workspace_policy: "stash")

      File.cd!(source_repo, fn ->
        assert {:ok, workspace} = Workspace.create_for_issue(%{id: "78", identifier: "MT-78"})
        File.write!(Path.join(workspace, "dirty.txt"), "dirty\n")
        assert {:ok, ^workspace} = Workspace.create_for_issue(%{id: "78", identifier: "MT-78"})
        refute File.exists?(Path.join(workspace, "dirty.txt"))
        assert {stash_output, 0} = System.cmd("git", ["-C", workspace, "stash", "list"])
        assert stash_output =~ "symphony-auto-stash"
      end)
    after
      File.rm_rf(test_root)
    end
  ...
  defp write_worktree_workflow!(workspace_root, opts \\ []) do
    reset_policy = Keyword.get(opts, :reset_dirty_workspace_policy, "fail")
    ...
    workspace:
      ...
      reset_dirty_workspace_policy: #{reset_policy}
  ...
  test "config reads defaults for optional settings" do
    ...
    assert config.acpx.json_strict == true
    assert config.acpx.suppress_reads == true

    write_workflow_file!(Workflow.workflow_file_path(),
      acpx_executable: "acpx --verbose"
    )

    assert Config.settings!().acpx.executable == "acpx --verbose"
  ...
  test "config resolves $VAR references for env-backed secret and path values" do
    ...
    write_workflow_file!(Workflow.workflow_file_path(),
      ...
      acpx_executable: "#{acpx_bin}"
    )

    config = Config.settings!()
    assert config.tracker.api_key == api_key
    assert config.workspace.root == Path.expand(workspace_root)
    assert config.acpx.executable == "#{acpx_bin}"
  end
  ...
  test "schema parse normalizes policy keys and env-backed fallbacks" do
    ...
    assert {:ok, settings} =
             Schema.parse(%{
               ...
               acpx: %{approve_mode: "approve-reads"}
             })

    assert settings.tracker.api_key == nil
    assert settings.workspace.root == Path.join(System.tmp_dir!(), "symphony_workspaces")

    assert settings.acpx.approve_mode == "approve-reads"
