...
defmodule SymphonyElixirWeb.Presenter do
    ...
    case Orchestrator.snapshot(orchestrator, snapshot_timeout_ms) do
      %{} = snapshot ->
        ...
        %{
          ...
          tokens: %{
            input_tokens: get_in(snapshot, [:agent_totals, :input_tokens]) || 0,
            output_tokens: get_in(snapshot, [:agent_totals, :output_tokens]) || 0,
            total_tokens: get_in(snapshot, [:agent_totals, :total_tokens]) || 0,
  ...
  @spec refresh_payload(GenServer.name()) :: {:ok, map()} | {:error, :unavailable | :timeout}
  ...
  defp projection_from_tracker(%TrackerIssue{} = issue, snapshot) do
    ...
    base = %{
      ...
      last_event: running_entry && running_entry.last_agent_event,
  ...
  defp running_entry_tokens(entry) do
    %{
      input_tokens: Map.get(entry, :agent_input_tokens, 0),
      output_tokens: Map.get(entry, :agent_output_tokens, 0),
      total_tokens: Map.get(entry, :agent_total_tokens, 0)
    }
  end
